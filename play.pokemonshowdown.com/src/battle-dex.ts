...
export const Dex = new class implements ModdedDex {
	...
	getPokemonIcon(pokemon: string | Pokemon | ServerPokemon | Dex.PokemonSet | null, facingLeft?: boolean, tera?: string) {
		if (pokemon === 'pokeball') {
		...
		let left = (num % 12) * 40;
		let style = "";
		if ((pokemon as Pokemon | ServerPokemon)?.fainted) {
			if (tera) {
				style = `;opacity:.7;filter:grayscale(100%) brightness(.5) ${TypeGlowStyles[tera.toLowerCase()]}`;
			} else {
				style = `;opacity:.3;filter:grayscale(100%) brightness(.5)`;
			}
		} else {
			style = tera? `;filter:${TypeGlowStyles[tera.toLowerCase()]}` : ``;
		}
		return `background:transparent url(${Dex.resourcePrefix}sprites/pokemonicons-sheet.png?v18) no-repeat scroll -${left}px -${top}px${style}`;
	}
